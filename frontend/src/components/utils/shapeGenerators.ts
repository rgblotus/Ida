...
export type ParticleShape = 'sphere' | 'cube' | 'torus' | 'helix' | 'galaxy' | 'wave' | 'random'
 ...
 */
export function generateSphere(count: number, radius: number = 10): ParticleData {
 ...
 */
export function generateTorus(count: number, majorRadius: number = 8, minorRadius: number = 3): ParticleData {
    const positions = new Float32Array(count * 3)
    const colors = new Float32Array(count * 3)
    const sizes = new Float32Array(count)
    ...
    for (let i = 0; i < count; i++) {
        ...
        positions[i3 + 1] = (majorRadius + minorRadius * Math.cos(v)) * Math.sin(u)
 ...
 */
export function generateHelix(count: number, radius: number = 5, height: number = 15, turns: number = 5): ParticleData {
    const positions = new Float32Array(count * 3)
    const colors = new Float32Array(count * 3)
    const sizes = new Float32Array(count)
 ...
 */
export function generateGalaxy(count: number, radius: number = 10, arms: number = 3): ParticleData {
    const positions = new Float32Array(count * 3)
    const colors = new Float32Array(count * 3)
    const sizes = new Float32Array(count)
 ...
 */
export function generateWave(count: number, width: number = 15, amplitude: number = 3): ParticleData {
    const positions = new Float32Array(count * 3)
    const colors = new Float32Array(count * 3)
    const sizes = new Float32Array(count)
 ...
 */
export function generateRandom(count: number, spread: number = 15): ParticleData {
    const positions = new Float32Array(count * 3)
    const colors = new Float32Array(count * 3)
    const sizes = new Float32Array(count)
 ...
 */
export function generateParticleShape(shape: ParticleShape, count: number): ParticleData {
    switch (shape) {
        case 'sphere':
        ...
        case 'helix':
            return generateHelix(count)
        case 'galaxy':
            return generateGalaxy(count)
        case 'wave':
            return generateWave(count)
        case 'random':
        default:
